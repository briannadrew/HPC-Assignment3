...
int main (int argc, char** argv) {
  ...
  */
  ...
  float max = init_val; // largest number in array
  ...
  float max_1 = init_val, max_2 = 0, max_3 = 0, max_4 = 0;
  ...
  do {
    do {
      ...
      for (int m = 0; m < block_size; m++) {
        for (int n = 0; n < block_size; n++) {
          MPI_Bcast(&r1[m*block_size+n], 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
          MPI_Bcast(&r2[m*block_size+n], 1, MPI_FLOAT, 1, MPI_COMM_WORLD);
          MPI_Bcast(&r3[m*block_size+n], 1, MPI_FLOAT, 2, MPI_COMM_WORLD);
          MPI_Bcast(&r4[m*block_size+n], 1, MPI_FLOAT, 3, MPI_COMM_WORLD);
        }
      }
